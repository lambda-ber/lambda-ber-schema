...
### RO-Crate Validation
Validate an `ro-crate-metadata.json` (or a crate directory) against the LAMBDA Core RO-Crate profile:
```bash
uv run lambda-ber-schema rocrate validate <crate_or_metadata_file>
# or: make validate-rocrate CRATE=<crate_or_metadata_file>
```
`linkml-validate` does not work on a crate. The checker lives in `src/lambda_ber_schema/rocrate/validation.py` and runs three layers: document shape (JSON Schema), per-entity closed-class checks dispatched on `@type`, and cross-entity graph rules. `tests/test_rocrate_profile.py` runs the same checker over the fixtures in `tests/data/rocrate/`. The JSON Schema it needs is generated by `make gen-rocrate`, which writes it to both `assets/rocrate/jsonschema/` and `src/lambda_ber_schema/schema/` so an installed package works without the `linkml` generators.
