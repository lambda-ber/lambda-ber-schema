
# Enum: StructuralFeatureTypeEnum

Types of structural features

URI: [lambda:StructuralFeatureTypeEnum](http://w3id.org/lambda/StructuralFeatureTypeEnum)


## Permissible Values

| Text | Description | Meaning | Other Information |
| :--- | :---: | :---: | ---: |
| alpha_helix | Alpha helix |  |  |
| beta_sheet | Beta sheet |  |  |
| beta_strand | Beta strand |  |  |
| turn | Turn structure |  |  |
| coil | Random coil |  |  |
| disordered_region | Intrinsically disordered region |  |  |
| transmembrane_helix | Transmembrane helix |  |  |
| signal_peptide | Signal peptide |  |  |
| transit_peptide | Transit peptide |  |  |
| domain | Protein domain |  |  |
| repeat | Sequence repeat |  |  |
| zinc_finger | Zinc finger motif |  |  |
| zinc_binding | Zinc binding site |  |  |
| coiled_coil | Coiled coil |  |  |
| motif | Structural motif |  |  |
| cavity | Structural cavity |  |  |
| channel | Molecular channel |  |  |
| pore | Molecular pore |  |  |
| hinge | Hinge region |  |  |
| linker | Linker region |  |  |
